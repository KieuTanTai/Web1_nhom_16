...
function slidesHandler(...names) {
  ...
  names.forEach((name) => {
    if (name.includes("news")) {
      ...
      if (nameSlide) behaviorSlides(parent, nameSlide, 3);
    }
  });
}
...
function scrollToHandler(nameStaticPage) {
  ...
  if (!staticPage && nameStaticPage === "news") {
    ...
    if (urlHandler("/", nowPath)) renderDOMHandler("homepage");
...
async function renderDOMHandler(nameDOM, ...requestRests) {
  ...
  try {
    ...
    if (nameDOM === "detailProduct" && requestRests)
      scriptDOM = await Bridge.promiseDOMHandler(
        `${originPath}/detail_product.html`
      );
    ...
    if (nameDOM === "detailProduct" && requestRests) {
      ...
      let format = product.format,
        type = product.type,
        genre = product.genre;
      let productAuth = product.author,
