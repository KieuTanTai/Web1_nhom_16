...
//   )}index.html?query=${bookName ? bookName : ""}`;
...
//       displayProducts(productList, query)
//     )
//   );
// }
...
function searchBtn() {
  ...
  searchBtn.addEventListener("click", () => {
    const bookName = searchInput?.value.trim(); // Lấy giá trị từ input nếu có
...
function renderSearchDOM(bookName) {
  ...
  let newURL = `${location.href.slice(
    0,
    location.href.lastIndexOf("/") + 1
  )}index.html?query=${encodeURIComponent(bookName || "")}`;
