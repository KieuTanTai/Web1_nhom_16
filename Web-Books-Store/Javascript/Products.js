...
import { renderDOMHandler } from "./Actions.js";
  ...
  try {
    ...
    arrayChild?.forEach((child, index) => {
      ...
      block.addEventListener("click", () =>
        renderDOMHandler("detailProduct", list[index])
      );
    });
  } catch (error) {
    console.error(error);
  ...
  if (!container) {
    ...
    containers.forEach((container) => {
      ...
      if (wrapper && containerID === "fs-container")
        ...
        list = productsList
          .filter((product) => product.type === "light novel")
          .toSpliced(5);
      else if (wrapper && containerID === "manga-container")
        list = productsList
          .filter((product) => product.type === "manga")
          .toSpliced(5);
